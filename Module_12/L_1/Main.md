...
Главное отличие заключается в структуре TCP-флагов (или использовании другого протокола в случае UDP), которые отправляет nmap для того, чтобы спровоцировать операционную систему жертвы на ответ.  

SYN (Half-open / Полуоткрытое): nmap отправляет классический пакет инициализации соединения с установленным флагом SYN. Однако, получив ответ, он не завершает процедуру рукопожатия (Three-way handshake) пакетом ACK, а сразу обрывает соединение пакетом RST. Это позволяет не регистрировать сессию в логах приложений.  

FIN (Уловка «Конец связи»): nmap отправляет TCP-пакет только с одним установленным флагом FIN (запрос на закрытие соединения), полностью игнорируя логику, по которой перед закрытием сессии её нужно сначала открыть.  

Xmas (Новогодняя елка): nmap отправляет пакет, в котором одновременно включены флаги FIN, PSH и URG. Пакет называется так потому, что в Wireshark чередующиеся биты флагов «мигают», как гирлянда на елке. По спецификации RFC 793 такой пакет является некорректным для закрытого соединения.  

UDP-сканирование: Принципиально отличается тем, что работает на транспортном протоколе UDP, где нет флагов, подтверждений доставки и понятия «сессия». nmap просто отправляет пустой (или специфичный для сервиса) UDP-дейтаграмм на целевой порт.  

- Как отвечает сервер?

Ответ целевой ОС зависит от состояния порта (открыт/закрыт) и соответствия сетевого стека Linux стандартам RFC.  
...
Команда записи дампа трафика: sudo tshark -i eth0 -f "host IP-target" -w nmap_attacks.pcap
